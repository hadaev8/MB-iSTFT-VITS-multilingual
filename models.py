...
class iSTFT_Generator(torch.nn.Module):
    def __init__(self, initial_channel, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel,
                 upsample_kernel_sizes, gen_istft_n_fft, gen_istft_hop_size, gin_channels=0):
...
class Multiband_iSTFT_Generator(torch.nn.Module):
    def __init__(self, initial_channel, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel,
                 upsample_kernel_sizes, gen_istft_n_fft, gen_istft_hop_size, subbands, gin_channels=0):
...
class SynthesizerTrn(nn.Module):
    ...
    def __init__(self,
        ...
        if mb_istft_vits == True:
            ...
            self.dec = Multiband_iSTFT_Generator(
                inter_channels, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes,
            ...
            print('iSTFT-VITS')
            self.dec = iSTFT_Generator(inter_channels, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates,
                                       upsample_initial_channel, upsample_kernel_sizes, gen_istft_n_fft, gen_istft_hop_size, gin_channels=gin_channels)
        else:
    ...
    def expand_speaker_emb(self, num_to_add=0):
        if hasattr(self, 'emb_g') and num_to_add > 0:
            self.emb_g.weight = nn.Parameter(
                torch.cat([self.emb_g.weight.data,
                           torch.randn((num_to_add, self.emb_g.weight.data.shape[1]), device=self.emb_g.weight.data.device)], dim=0))
            self.emb_g.num_embeddings += num_to_add
